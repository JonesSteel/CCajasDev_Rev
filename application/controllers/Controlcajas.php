...
class Controlcajas extends CI_Controller
{
    ...
    public function guardaRegistroRemesa(){
        ...
        if($this->validaExisteRegistroRemesa($_POST) == false){
        	...
        	// Esta parte es dada por el login de cada usuario y el módulo al que este casado
            $_POST['id_usuario'] = $this->session->userdata['idUsuario'];

            // Este parametro puede ser modificado
            $_POST['anio_remesa'] = date('Y', strtotime('-1 year'));
            ...
            if($resInsertRemesa == true) {
				...
				echo "<p id='profile-name' class='profile-name-card'><font color='red'>NO SE PUDO GUARDAR LA INFORMACIÓN DE LA REMESA</font></p>";
            }
        }
    }

    public function guardaRegistroRemesa2020() {
		if($this->validaExisteRegistroRemesa($_POST) == false){
    ...
    public function generaNombre($infoRemesa){
        ...
        $nombreRemesa = "R".$infoRemesa['numero_remesa']."-".$infoRemesa['anio_remesa']."-".$infoRemesa['id_modulo'];
        return $nombreRemesa;
    }
